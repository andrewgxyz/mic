...
use super::{
  ...
  data::string_clean,
  date::{get_start_end_week_dates, parse_string_to_datetime, parse_string_to_yearless_date},
};
  ...
  pub fn matches(&self, song: &SongData) -> bool {
    ...
    matches.push(self.genre.as_ref().map_or(true, |genres| {
      genres
        .split(',')
        .map(|m| m.to_string())
        .any(|genre| song._genre.contains(&genre))
    }));
    ...
    if self.instrumental && song._lyrics.len() == 0 {
      matches.push(true);
    }
